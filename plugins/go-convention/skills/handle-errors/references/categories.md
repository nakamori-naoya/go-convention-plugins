...
| 利用上限に達した | `ErrRateLimited` | サービスを守るために入口が課す、頻度の上限に当たった |
...
「利用上限に達した」は、サービスを守るために入口が課す、時間当たりの回数のような頻度の上限だけに使う。業務の資料には現れず、入口の頻度制限が返す。

## 「回復不能」と、一時的な失敗

「回復不能」は、再試行しても直らず、運用者がデータか環境を直すまで解決しないものだけに使う。設定や環境の不備、保存されたデータの破損、外部の依存先が契約を破った応答がこれに当たる。依存先の一時的な不在（「依存先が利用できない」）、時間切れ、競合には使わない。「分類が分からない」を理由に選ばない。分からなければ、分類に当てずに返し、境界に「分類不能」と判定させる。

## 「分類不能」を土台にしてよい場面

「分類不能」を自分で土台にしてよいのは、値オブジェクトを経た正しい呼び方では起こりえない、呼び方の間違いだけである。トランザクションの無い ctx でリポジトリを呼んだ、入れ子のトランザクションを張ろうとした、がこれに当たる。業務の拒否を「分類不能」にしない。
