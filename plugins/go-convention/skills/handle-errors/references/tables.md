...
網羅は静的検査で守るので、本番でこの経路に来るのは、表の更新が漏れたときだけである。
...
三つの表が15の分類を過不足なく持つことは、分類の定義と表の行を突き合わせる静的検査（プロジェクト固有の lint）で守る。テストのためだけに分類の一覧を公開すると、使われない公開面が増えるので、網羅をテストで確かめる形はとらない。実装の側に、分類の一覧を返す関数を足さない。
