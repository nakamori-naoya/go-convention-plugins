...
**再処理で通りうるか**は、同じ要求をもう一度処理すれば通る見込みがあるかである。ワーカーと受信境界は、これで要求をもう一度処理させるか（再処理）を決める。物理設計の「再試行」（一つのトランザクションの中でのやり直し。usecase が物理設計の指定を渡す）とは別のことで、語を分けている。技術処理のライフサイクルが定める二つの終わり方（一時失敗と恒久失敗）のどちらで終えるかも、これで決める。「はい」なら一時失敗、「いいえ」なら恒久失敗である。ライフサイクルの型そのものは、データモデルの資料が決める。
