...
- 資料の一つの BDD を `id` に写すのは、repository 全体でそれを主に担う一つのテストだけである（apply-go-test-convention の規則）。ほかのレベルが主に担う BDD の場面を通すなら、生成した id を使う
- `name` と `description` は言い換えない。`description` の `Then:` に写せない行があっても削らない（§2）

## 5. 資料の宣言と、どのテストも担わない BDD

資料の ID を写したテストファイルには、`// BDD の資料: <repository 相対の path>` を一つだけ書く。ドメインのテストが主に担わない BDD（同時実行、別の集約、usecase の手順）は、この package では何も書かない。それらを主に担うテストが、別の package で名乗る。どのテストも担わない BDD だけを、apply-go-test-convention の形で列挙する。
