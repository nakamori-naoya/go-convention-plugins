...
ワーカーの巡回と、集約を作らず手順として書く usecase（単純なパターン）は、一件ずつ処理して飲み込む失敗だけを記録する。
...
ワーカーの巡回のループを誰が持つかは、巡回の相手で決まる（implement-handler が定める）。相手が単純なパターンの手順なら、ループと失敗の閉じ込めは手順が持ち、巡回は一回の巡回で手順を一度呼ぶだけにする。記録も、飲み込む場所である手順が行う。次の例は、延滞の通知の要求（貸出を延滞にしたときに同じトランザクションで記録しておく Outbox の行）を、一件ずつ回収して送る手順のループである。
