...
logger.LogAttrs(ctx, slog.LevelError, "保存値の壊れた貸出を一覧から除いた",
	slog.String("loan_id", row.ID.String()),
	slog.Any("err", err),
)
```

何を出して何を伏せるかは、その行で決める。値オブジェクトに `slog.LogValuer` も `String()` も実装しない。型に実装すると、何が出るかが記録する行から見えなくなり、`%v` や `fmt.Errorf` を通して意図しない場所でも中身が出る。ドメインは `log/slog` を import しない。
