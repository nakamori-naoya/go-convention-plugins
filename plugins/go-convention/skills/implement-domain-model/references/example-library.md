...
図書館の貸出のドメインモデル（集約「貸出」、状態は貸出中・延滞・返却済み）を写した、ドメインの package の全体である。書き始める前に一度読む。写すのは形である。値オブジェクトと状態ごとの型の分け方、和型のコマンドと `<コマンド名>Result` の返り値、具体エラーの置き方がこれに当たる。写さないのは題材である。型とメソッドの名前、状態の数、コマンド、拒む理由は、対象のドメインモデルと業務知識の資料から取る。

名前と引数は、図書館の貸出を題材にしたドメインモデルと業務知識の見本の資料に合わせている。業務の時刻の引数は、時点で受ける（「本を借りる」は貸出日時、「延滞にする」は判定日時）。資料が業務知識への提案に回しているものは書かない（[資料の読み方](reading-the-model.md) の「提案の段階のもの」）。見本の資料は、延滞と返却済みの貸出が「延滞にする」を受けたときの拒む理由を提案に回しているので、その二つのメソッドは書いていない。貸出番号も見本の資料では提案に回っているが、保存した貸出を見分ける識別子なので、同じ節の例外に従って提案の語で書いている。
...
// MarkOverdue は「延滞にする」。延滞と返却済みが受けたときの拒む理由を資料が持たないので、
// 和型の Loan には置かず、貸出中の型だけが持つ。
