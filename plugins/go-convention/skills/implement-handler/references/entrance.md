...
要求のプリミティブは、入口で値オブジェクトへ変換してから、usecase の入力に詰める。usecase は検証済みの値しか受け取らない。`New*` が拒んだエラーは、入口がそのまま返す。「入力が不正」に分類されているので、応答の Code は、handle-errors の応答の表（エラーの分類から `connect.Code` を決めた表）で決まる。
...
ワーカーの巡回のループを誰が持つかは、巡回の相手で決まる。相手が戦術的 DDD の command なら、巡回（入口）が query の usecase で候補を選び、一件ごとに command の usecase を呼び、ループと失敗の閉じ込め（処理の表の「全体に及ぶか」）を持つ。command の usecase は一つの集約だけを扱い、読み取りのポートを呼ばないからである。相手が単純なパターンの手順（Outbox の回収と発行など）なら、回収と一件ごとの確定と失敗の閉じ込めは手順が持つので、巡回は一回の巡回で手順の `Execute` を一度呼ぶだけにし、ループを書かない。両方でループを書くと、失敗の閉じ込めと記録が二重になる。
