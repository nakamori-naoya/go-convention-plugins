...
|---|---|---|---|
| handler のテストが主に担う BDD（認証、認可、応答の Code）がある | 資料の ID そのまま。テストファイルに資料の宣言を一つ書く | 資料の見出し文 | 資料の gherkin ブロックを改変せず転記 |
| 無い | 生成した id（ディレクトリ内で一意・不変） | 業務語で 1 文（「期限を過ぎた仮押さえは確定できない」） | `Given:` 前提の行と主体 / `When:` 誰がどの RPC を何で呼ぶ / `Then:` Code と文言、成功なら応答と反映 |

主に担うかは apply-go-test-convention の規則で決める。ほかのレベルが主に担う BDD の場面を公開 API から見るケースでは、id を生成する。
