...
規約を満たしたテストファイルを一つ置く。書き始める前に一度読む。写すのは形である。テーブルの struct のフィールドの並び、ループ本体に一回だけ書く実行と検証、`require` と `assert` の使い分けがこれに当たる。写さないのは題材である。テスト関数と型の名前、BDD の ID、値は、対象の資料から取る。

コードは、リポジトリの `tests/examples/` にある Go のモジュール（Go 1.27）と同じで、`go vet` と `go test -shuffle=on` が通る。テストの対象は、図書館の貸出の「本を借りる」だけを持つ小さな package で、例を小さく保つために、エラーの分類の package を使わず標準の `errors` で具体エラーを定義している。
