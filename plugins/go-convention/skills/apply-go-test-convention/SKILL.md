...
## 差し替えてよいのは、制御できない外部の境界、時計、採番の三つだけ

外部の API、配信基盤への発行、認証基盤のトークンの検証のようにテストの中で望む状態を作れない相手と、時計と、採番器だけを、ポートの interface から `mockgen` で生成した gomock で差し替える。手書きの偽物（固定の時計、インメモリのリポジトリ）と、外部の API を `httptest` で立てる stub は作らず、要ると感じたらポートの切り方を疑う。同じプロセスの部品（usecase、リポジトリ、トランザクションの管理、interceptor、認可の方針の表）は、本番の組み立ての関数で実物を使う。内部を差し替えると、組み立ての誤りをテストが見逃すからである。失敗は、実物で起こせるもの（制約の違反、行が無い、古い版での更新）は実物を壊れた状態にして起こし、起こせないものだけを mock に返させる。
