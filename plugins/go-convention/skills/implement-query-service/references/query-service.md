...
Query の実装は、読み取り専用の接続（各接続の既定を読み取り専用にした pool）だけから読む。ctx のトランザクションを見ない。「ctx にトランザクションがあれば乗り、無ければ pool で読む」という切り替えを書かない。command は読み取りのポートを呼ばないので、Query の実装がトランザクションの中で呼ばれる経路は無い。切り替えを書くと、トランザクションの張り忘れが黙って通る。
