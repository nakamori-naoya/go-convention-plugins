...
過去の実装、設定、API、データの形との互換は保たない。いまの要求に対して最も単純で一貫した形へ、直接移す。旧い名前の別名、旧い設定の読み替え、旧い形だけを通す分岐、新旧の二重の読み書きを書かない。変えるときは、呼び出し側、テスト、設定、文書を同じ変更で新しい形へ揃える。互換を残すと、同じことをする経路が二つになり、どちらが正しいかを読み手が毎回確かめることになるからである。

## 有限の値の文字列を、あちこちに書かない

状態や種別のような有限の値を、実装、テスト、Builder の中に裸の文字列で繰り返し書かない。値を所有する package に一度だけ定義し（封じた型の package 変数か、名前付きの定数）、DB の生成型へ写す変換もテストの期待も、その定義を使う。同じ値の綴りが何か所にもあると、一か所を直したときに残りが黙って食い違うからである。
