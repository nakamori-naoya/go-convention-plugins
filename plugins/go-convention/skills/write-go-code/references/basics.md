...
動作が暗黙だと、失敗の原因が、失敗した行から遠い場所へ移る。「この返却期限はなぜ14日後なのか」に答えるために実装を辿る時間が、暗黙一つごとに増える。
...
値を丸めると、失敗が成功に見える。DB の行が壊れていても、入力が欠けていても、丸めた瞬間に痕跡が消える。
...
フォールバックを書くと「失敗したのに動いている」状態になり、失敗に気づくのが利用者からの問い合わせまで遅れる。要るかどうかは業務と運用の判断で、コードを書いている最中に決めることではない。
...
ガード節で書けば、拒む理由と拒む行が隣り合う。上から読むと、資料の拒む理由の順に並んでいる。
...
`return x, err` と値を書く。名前付きの結果は、`defer` で結果を書き換えるときだけ使い、そのときも `return` には値を書く。naked return だと、「この行で何が返るか」を知るのに関数の先頭まで戻って読むことになる。
...
`ErrInvalidSetting` は環境の不備なので、「回復不能」の分類で定義する。設定値が値オブジェクトとして不正なときの分類の付け替えは、handle-errors が決める。
...
使われない宣言があると、読み手は「どこで使うのか」を探すことになる。
