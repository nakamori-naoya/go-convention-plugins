...
**しない:** 型変換・既定値の補完・初期化を、読み手が知らない場所（`init`、struct のゼロ値の特別扱い、埋め込みによるメソッドの昇格、`String()` による暗黙の文字列化）で起こす。環境で変わらない値（業務の数・規則・固定の名前）を設定 file や環境変数へ逃がす（環境変数で受けるのは接続先・ポート・鍵・ログの出力先のように配備環境で変わる値だけで、その読み方は handler の規約が決める）。
...
ctx を受け取るのは、層を問わず、I/O（DB・ネットワーク・外部プロセス・時刻待ち）を行うか、それを行う関数を呼ぶ関数だけである。ドメインの型がそうであるように、usecase や handler にある純粋な変換・計算の補助関数も ctx を受け取らず、使われない ctx は第一引数へ移さず引数ごと外す。呼ぶ側の interface がシグネチャに ctx を要求する場合だけ、使わなくても第一引数に置く。
...
## 15. いまの呼び出しが要らないものを書かない

**する:** 関数・引数・フィールド・分岐・オプションは、対象範囲のテストか呼び出し側が要求するものだけ書く。要るときに要る人が書く。
**しない:** 空の関数、`TODO` だけの本文、使われない引数を `_ = x` で黙らせる、「後で使う」設定キーや `opts ...Option`、いま呼ばれない分岐。

理由: 使われない宣言は読み手に「どこで使うのか」を探させる。`unused` は既定の設定では非公開の関数・型・フィールドしか落とさず、公開シンボルと引数は規則で守るしかない。
