...
| 3 | 厚さ | 対象 usecase の 5 観点に実在する相異なる配線分岐をケースにする。1 ケースで複数観点を区別できる場合は兼ね、同じ結果経路へ入力値だけを変えるケースは書かない | [five-viewpoints.md](references/five-viewpoints.md) |
...
- 資料が未決と明示した値がfixtureの入力または期待DB行に要る → 仮値を置かない。未決事項、影響する列とケース、値を確定できる資料または判断、確定後に再開する最初の工程を返す

止まるときは、書いた範囲と書かなかった範囲を分け、返す先（資料、実装の規約、利用者）と必要な決定を報告に示す。

判断の揺れでは、その時点の根拠から最も筋の良い形を仮説として採り、仮説であることと採らなかった形を報告に明示して進む。

- 5観点のどのケースを先に書くか、`name`の文言が一意に決まらない: 観点の表の順に最も近い形を採り、報告に示す。

## チェックリスト

- [ ] 各ケースの `name` から 5 観点のどれかが読める。書かない観点の表に当たるケースが無い
- [ ] 対象実装の 5 観点にある相異なる配線分岐を各ケースから観測できる。同じ結果経路へ入力値だけを変えたケースが無い
...
- [ ] 資料の ID を写したのは usecase のテストが主に担う BDD だけで、テストファイルに資料の宣言がある。それ以外は生成した id
