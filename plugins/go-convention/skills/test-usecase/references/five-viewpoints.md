...
| 観点 | ケースを分ける条件 | 重複させない条件 |
|---|---|---|
| 入力解決 | 異なる変換または解決先があり、到達先や返す sentinel を別に観測する必要がある | 同じ変換経路へ値だけを変えた拒否 |
| 複数集約の協調 | 読む相手と、読んだ事実によって変わる結果が異なる | 同じ相手・同じ結果経路の値違い |
| ソース選択 | 入力によって読む先が分かれ、選ばれたソースを別に観測する必要がある | 同じソース内の SQL 条件違い |
| tx 境界 | commit 後の可視性と、途中失敗時の全未反映を区別する。異なる書き込み経路が別の原子性境界を持つならそれぞれ扱う | 同じ `Run` と同じ書き込み経路で失敗点だけを変えたもの |
| 呼び分け | 成功、保存前の拒否、`(Result, bool)` の false など、永続化へ進むかと選ぶ操作が異なる | 同じ操作・同じ保存結果になる状態値違い |
...
- 同じ入力変換と同じ到達停止を観測済みなら、`CustomerID: ""` のように値だけを変えたケースは足さない。別の変換関数・sentinel・到達先を持つなら別の配線として扱う
...
- 資格の導出そのもの（30 日・3 回・14 日）はドメイン層の `NewEligibility` のテストが見る。ここでは「その顧客の記録だけが渡った」ことを、本人の記録が結果を変える経路と他人の記録が結果を変えない経路で区別する
- 重なりの判定（半開区間・隣接）はドメイン層の `Overlaps` のテストが見る。ここでは占有中の利用枠が渡ったと観測できる経路を扱い、境界値違いを増やさない
...
- 同じ `Run` と同じ書き込み経路では、失敗する手だけを変えて増やさない。別の書き込み経路や原子性境界がある場合は、その経路でも全未反映を観測できるかを別に判断する
