...
- `develop-go-unit` は、同packageの公開入口を隣接 `playbook.yml` の `steps[].skill` で「テスト → 赤 → 実装 → 緑 → 整える」の順に呼ぶ合成入口である。層（`unit.layer`）ごとのテストと実装の工程は、`when` で一つだけ実行する。`test-<layer>` と `apply-go-test-convention` は「実装が無い状態で資料のBDDから先にテストを書く」場面を通常の場面として自分の判断基準・手順・停止条件に持ち、`implement-<layer>` は「先に存在する赤いテストを緑にする」場面を持つ（どちらも合成入口の名前を書かない）。何を赤とみなすかは、どの skill も定義を持たず、development-convention の `develop-inside-out` の赤の定義に従う。合成入口の `SKILL.md` はその前提に乗り、呼ばれる側の判断を上書きせず、TDDの1単位としての判断基準（赤を確認せずに実装しない、緑になるまでテストを変えない、整えるときに振る舞いを変えない、資料のBDDを超えるテストを足さない）と、呼ぶ相手の入力と出力だけを持つ。工程順は `scripts/validate.sh` が構造検査する。
