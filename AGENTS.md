...
- 両runtimeのpackage manifestは `plugins/go-convention/skills/<name>/` の自己完結skill 18件（規約14件と、層ごとのTDD入口 `develop-<layer>` 4件）を直接公開し、同一に保つ。存在確認だけのprepare、外側の単一routing playbook、入口別runtime manifestを要求しない。各skill直下の`playbook.yml` v2を工程順序の定義とし、同じagentが`agent_work: invoking_agent`の工程を宣言順に実行する。
...
- `develop-<layer>` の4入口は、同packageの公開入口を隣接 `playbook.yml` の `steps[].skill` で「テスト → 赤 → 実装 → 緑 → 整える」の順に呼ぶ合成入口である。`test-<layer>` と `apply-go-test-convention` は「実装が無い状態で資料のBDDから先にテストを書き、コンパイルエラーで赤になる」場面を通常の場面として自分の判断基準・手順・停止条件に持ち、`implement-<layer>` は「先に存在する赤いテストを緑にする」場面を持つ（どちらも合成入口の名前を書かない）。合成入口の `SKILL.md` はその前提に乗り、呼ばれる側の判断を上書きせず、TDDの1単位としての判断基準（赤を確認せずに実装しない、緑になるまでテストを変えない、整えるときに振る舞いを変えない、資料のBDDを超えるテストを足さない）と、呼ぶ相手の入力と出力だけを持つ。工程順は `scripts/validate.sh` が構造検査する。
