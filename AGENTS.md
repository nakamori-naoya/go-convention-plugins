...
- 両runtimeのpackage manifestは `plugins/go-convention/skills/<name>/` の自己完結skill 18件（規約14件と、層ごとのTDD入口 `develop-<layer>` 4件）を直接公開し、同一に保つ。存在確認だけのprepare、外側の単一routing playbook、入口別runtime manifestを要求しない。各skill直下の`playbook.yml` v2を工程順序の正本とし、同じagentが`agent_work: invoking_agent`の工程を宣言順に実行する。
