...
- 公開入口は `plugins/go-convention/playbooks/go-convention/<name>/` の 13 件で、それぞれ同名の内部 skill `plugins/go-convention/skills/<name>/` へ 1 工程でルーティングするだけにし、内容を持たない。入口の一覧は両 runtime の manifest（`metadata.harness.playbooks` / `internalPlugins`）が正本で、Codex と Claude の manifest は同一に保つ。
