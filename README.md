...
- コマンドデータモデルの BDD を、Before を作り、操作し、After に挙がったテーブルを突き合わせるリポジトリのテストにしたい。クエリデータモデルの BDD を、Before を作って読み、取得結果と突き合わせる Query の実装のテストにしたい
...
```text
このリポジトリのテストを command-data-model 資料の BDD から書いて。After に挙がったテーブルを突き合わせて。
```
